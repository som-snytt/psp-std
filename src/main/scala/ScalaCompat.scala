...
trait CompatSeq[Repr, CC[X], A] extends sc.GenSeqLike[A, Repr] {
  ...
  private[this] def wrapOp(f: Elemental[A] => Foreach[A]): Repr      = f(wrap(repr)).foldl(newBuilder)(_ += _).result
  private[this] def wrapMap[B](f: Elemental[A] => Foreach[B]): CC[B] = ???
  ...
  def indexWhere(p: A => Boolean,from: Int): Int                                                            = { forxs.foreachWithIndex((x, i) => if (i >= from && p(x)) return i else false) ; NoIndex }
  ...
  def ++[B >: A, That](that: sc.GenTraversableOnce[B])(implicit bf: CBF[Repr,B,That]): That   = bf() ++= forxs.toTraversable ++= that.seq result
  ...
  def dropWhile(pred: A => Boolean): Repr                                                     = wrapOp(_ dropWhile pred)
  def filter(pred: A => Boolean): Repr                                                        = wrapOp(_ filter pred)
  def filterNot(pred: A => Boolean): Repr                                                     = wrapOp(_ filterNot pred)
  ...
  def scanLeft[B, That](z: B)(op: (B, A) => B)(implicit bf: CBF[Repr,B,That]): That           = (bf() ++= forxs.scanl(z)(op).toTraversable).result
  ...
  def span(pred: A => Boolean): (Repr, Repr)                                                  = takeWhile(pred) -> dropWhile(pred)
  ...
  def takeWhile(pred: A => Boolean): Repr                                                     = wrapOp(_ takeWhile pred)
  ...
  def count(p: A => Boolean): Int                                                  = forxs.foldl(0)((res, x) => if (p(x)) res + 1 else res)
  def exists(pred: A => Boolean): Boolean                                          = forxs exists pred
  def find(pred: A => Boolean): Option[A]                                          = forxs find pred
  ...
  def forall(pred: A => Boolean): Boolean                                          = forxs forall pred
  ...
  def toTraversable: sc.Traversable[A]                                             = forxs.toTraversable
