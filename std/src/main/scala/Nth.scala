...
object Nth extends (Int => Nth) {
   ...
   */
  final class Impl private[std] (val value: Int) extends AnyVal with IndexOrNth with api.Nth {
    // type This = Nth
  ...
  def undefined                    = new Impl(-1)
  def fromIndex(index: Index): Nth = apply(index.value + 1)
  def apply(value: Int): Nth       = if (value < 1) undefined else new Impl(value)
