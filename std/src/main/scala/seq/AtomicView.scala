...
final case class Sized       [A   , Repr](prev: BaseView[A, Repr], override val sizeInfo: PreciseSize)  extends CompositeView[A, A, Repr](pp"sized $sizeInfo",  _ => sizeInfo) with HasPreciseSize { def isEmpty = sizeInfo.value == 0L }
