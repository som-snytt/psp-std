...
final class BitVector(longs: Array[Long], val sizeInfo: PreciseSize) extends Direct.DirectImpl[Boolean] {
  private def nubSize  = (sizeInfo % 64).getInt
  private def lastSize = if (isEmpty) 0 else if (nubSize > 0) nubSize else 64

  def apply64(index: Index): Bit64      = new Bit64(longs(index))
  def foreach(f: Boolean => Unit): Unit = sizeInfo.indices foreach (i => f(elemAt(i)))
