...
package gen {
  ...
  class TextGenerator(val letter: Gen[Char], charsInWord: SizeRange, wordsInLine: SizeRange) {
    def word: Gen[String]                   = charsInWord.chooseInt flatMap nLetters
    def line: Gen[String]                   = wordsInLine.chooseInt flatMap (n => listOfN(n, word)) map (_ mkString " ")
    def nLines(n: Int): Gen[Direct[String]] = directOfN(n, line)
    def nLetters(n: Int): Gen[String]       = listOfN(n, letter) map (_ mkString "")
  }
...
package object gen {
  ...
  def directOfN[A: Arb](n: Int, g: Gen[A]): Gen[Direct[A]] = containerOfN[Direct, A](n, g)(?, _.toScalaTraversable)
  def directOf[A: Arb](g: Gen[A]): Gen[Direct[A]]          = containerOf[Direct, A](g)(?, _.toScalaTraversable)
  def chooseFrom[A](xs: Direct[Gen[A]]): Gen[A]            = indexFrom(xs.indices) flatMap xs.elemAt
